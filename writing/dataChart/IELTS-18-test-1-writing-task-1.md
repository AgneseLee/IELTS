# IELTS 18 Test1 — Task 1 Revision Log
*2026-06-27*
...
Overall, there was a general upward trend in population living in cities for all countries. While the Philippines accounted for the lion's share in 1970, Malaysia, which saw a sharp increase over the past 50 years, surpassed it and became the most urbanized country by 2020.

Looking at the graph, the figure for the Philippines fluctuated for the first 4 decades, peaking at roughly 50% in 1990 before dropping to 46% in 2000. However, it climbed marginally to 50% by 2020, which stood in stark contrast to Malaysia's surge from 30% to roughly 75% within the same period. As for Indonesia, it followed a similar upward trend to Malaysia and climbed to the second position in 2020 with a figure of approximately 57%, whereas Thailand's figure remained relatively stable at 30% between 1990 and 2020.

In terms of predictions, all of the countries are predicted to maintain the upward trend in the next 20 years. Particularly, Thailand is predicted to see the steepest rise, climbing from approximately 35% to 50% by 2040, with Malaysia still ranking first by 2040 with a high of 85%.
...
|---|---|
| `up-comming` | `upcoming` |
| `with prediction` | `with predictions`（复数）|
| `figure of Philippines` | `figure for the Philippines`（介词 + 冠词）|
| `once having peaked... but subsequently dropped` | `peaking... before dropping` |
| `climbed marginally at 45%` | `climbed marginally to 50%`（介词）|
| `climbed up to` | `climbed to`（up 冗余）|
| `a steepest rise` | `the steepest rise`（最高级用 the）|

### 词汇误用

| 错误 | 修正 | 原因 |
|---|---|---|
| `Philippines` | `the Philippines` | 菲律宾国名固定加 the |
| `largest urbanization country` | `most urbanised country` | urbanization 是名词不是形容词 |
| `sharp surge` | `surge` | surge 本身已含急剧之意，sharp 冗余 |
| `shared a similar trend as` | `followed a similar trend to` | 固定搭配 similar to |
| `the number of Thailand` | `Thailand's figure` | 国名不用 "number of" |

### 数据错误

| 错误 | 修正 |
|---|---|
| Philippines 2000: `40%` | `46%` |
| Philippines 2020: `45%` | `50%` |
| Indonesia 2020: `50%` | `57%` |
| Thailand 预测起点: `30%` | `35%`（2020年现值）|
| Thailand `33% per year` | 删除——泰国20年共涨约15pp，不是每年33% |
