...
- `preview_session`：提取语料并返回预览，不修改文件。
...
- 每个主题生成 1 个 IELTS-ready collocation 和 1–2 个例句。
- collocation 以 `/ New collocation` 追加到对应 `### 语料` 的现有反引号行末尾。
- 例句以带标签的列表追加在同一 `### 语料` 中。
- 对 collocation 做大小写及尾部标点归一化去重。
- 不删除、不替换、不排序或重新格式化已有内容。
