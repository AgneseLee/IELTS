...
> Central index. Choose the task type first; read only the relevant note.
...
| Visual | Main decision | Notes |
|---|---|---|
| Bar chart | Static or Dynamic | Choose by time pattern, not shape |
| Line graph | Usually Dynamic | Focus on direction, peak and reversal |
| Pie chart | Usually Static | Focus on share, rank and gap |
| Table | Static or Dynamic | Read the time dimension first |
| Mixed charts | Group by data relationship | Do not create one body paragraph per visual |

## Universal Four-Paragraph Structure

1. Introduction: accurately paraphrase the task.
2. Overview: report two main features; avoid detail overload.
3. Body 1: group related information.
4. Body 2: cover the contrasting group, remaining area or later stages.
...
1. 词汇以"词"为单位记，不是以"搭配"为单位 → 写时想不起来
2. 段落结构没分组就开始写 → 衔接强行，逻辑混乱
3. 高频套词：`in stark contrast`、`in favor of`、`a more significant X%` 每篇重复
...
| 功能 | 用语 |
|---|---|
| 引入主体 | `Regarding X... / Looking at X... / In terms of X...` |
| 同类补充 | `Similarly, / Likewise, / In the same vein,` |
| 对比转折 | `Conversely, / By contrast, / This contrasted sharply with...` |
| 数据跟进 | `This was followed by... / A similar pattern was observed in...` |
| 例外 | `Notably, / One exception was... / X proved to be the outlier,` |

规则：每篇 `in stark contrast` 最多用一次，只用于最大的对比。
...
| 词 | 用于 | 例句 |
|---|---|---|
| `figure` | 具体数值 | `The figure peaked at 7,100.` / `The figure stood at 5,000.` |
| `number` | 图表介绍 & 绝对数量趋势 | `The number of closures rose.` / `illustrates the number of...` |
| `proportion` | 百分比 / 相对份额 | `The proportion of households with fridges reached 100%.` |
| `allocation` | 机构预算分配（警察/政府） | `The allocation for technology rose from 8% to 14%.` |

**常见错误：**
- ✗ `The graph illustrates the figure of shop closures.` → 介绍图表用 `number`
- ✗ `The proportion of shop closures was 5,000.` → 5,000 是绝对数量，不是比例
- ✗ `The allocation for shop closures remained stable.` → 店铺数量不是被分配的资源
- ✓ `The allocation for technology rose from 8% to 14%.` → 警察预算题：钱被明确拨给某项 ✓
