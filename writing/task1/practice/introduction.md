# IELTS Writing Task 1 数据图表 — Introduction 复盘
*2026-07-28*

> 用法：先看「本轮练习」，再看「高频错误」。需要专项复习时，直接跳到对应图表类型。
...
- [柱图](#柱图-bar-chart)
- [线图](#线图-line-chart)
- [混合图](#混合图-mixed-charts)
- [表格与饼图](#表格与饼图-table--pie-chart)
- [通用模板](#通用模板)
...
- `percentages of young people` 没有准确说明饼图的地点构成。
- `distribution in three types` 含义模糊，用 `participation in` 更自然。
- `divided into 2 age groups` 修饰对象不清楚。
- 正式写作中写 `two`，不写 `2`。

### 2. Little Chalfont Library
...
> The pie chart and table show the age distribution of members and the breakdown of loans by category at Little Chalfont Library in 2016, while the bar chart illustrates how the total number of loans changed between 2007 and 2016.

**问题**

- `line chart` → `bar chart`：图中使用柱形。
- `in a town called Little Chalfont` → `at Little Chalfont Library`：后者更直接。
- `the proportion of loans by category` 可以理解，但多个类别共同构成整体时，`breakdown` 更自然。
- `the number of loans` → `the total number of loans`：更精确。

**表达选择**

| 表达 | 侧重点 |
|---|---|
| `the proportions of loans by category` | 强调各项百分比 |
| `the breakdown of loans by category` | 强调整体如何被拆分 |
| `illustrates how...changed` | 清楚展示动态变化 |
| `illustrates changes in...` | 更紧凑，适合较长的 Introduction |
...
| 检查点 | 你的典型错误 | 修正原则 |
|---|---|---|
| 图表类型 | 把柱图写成 `line chart` | 先确认 bar、line、pie 或 table |
| 测量对象 | 把 percentage change 写成 price | 严格对应标题和 y 轴 |
| `distribution` | 看到百分比就使用 `distribution` | 先判断是 ownership、share、rate 还是 breakdown |
| 介词 | `distribution annual income` | 写成 `distribution of...` |
| 语序 | `how prices changed of three metals` | 用名词结构或把完整主语放在 `changed` 前 |
| 单复数 | 多张图仍写单数 `chart` | 根据图表数量选择单复数 |
| 年份 | `2001`/`2006` 等笔误 | 写完逐项抄回原图核对 |
| Introduction/Overview | Introduction 后直接接趋势和数据 | 换段后再写 `Overall` |

### 30秒检查清单

- [ ] 图表类型和单复数正确吗？
- [ ] `number / percentage / rate / share / breakdown` 与图中单位一致吗？
- [ ] 地点和年份准确吗？
- [ ] 有没有加入趋势、排名或具体数字？
- [ ] 一句话是否过长？需要拆成两句吗？
...
**问题：** 图表展示收入如何被支出，测量对象应为 `expenditure`，不是 `income`。
...
**问题：** `distribution` 不够精确；`its prediction` 指代不明；`up-comming` 应为 `upcoming`。
...
**核心：** 静态信息放第一句，动态信息放第二句；三张图不必强塞进一个长句。
...
- `transformation` 不能准确表示资金来源和支出分配。
- 正式财务语境使用 `income and expenditure`。
- 应明确是 `pie charts`。
...
| 题型 | 推荐模板 |
|---|---|
| 单一动态图 | `The line/bar chart illustrates how [measure] changed in [place] between [years].` |
| 单一静态图 | `The bar/pie chart compares [measure] across [categories/places] in [year].` |
| 两图、单位不同 | `The two charts compare [percentage measure] with [number/time measure] in [place] from [year] to [year].` |
| 静态＋动态混合图 | `The pie chart and table show [static information] in [year]. The bar chart illustrates changes in [dynamic measure] between [years].` |

## 最后提醒

Introduction 的目标不是“写高级”，而是用一到两句准确交代：

> **图表类型 + 测量对象 + 地点 + 时间**

其中最重要的是**测量对象**。写完后先问：

> 我描述的是图中直接测量的数据，还是我自己推断出来的概念？
