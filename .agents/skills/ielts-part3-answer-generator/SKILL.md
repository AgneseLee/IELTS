...
每道普通题都必须用 `For example` 或 `For instance` 引出短例，将已选逻辑链具体化。例句后必须紧接一句独立的 reflection，说明例子中的人物、行为或结果如何印证已选逻辑链；reflection 不得兼作最终总结，也不得新增原因、事实或观点卡。reflection 应直接延续具体情境，禁止使用 `This example shows ...`、`This shows how ...`、`It illustrates ...` 等讲解答案结构的元话语。可自动生成常识性的具体人物、场景、物品、地点、活动或类别；Q7 也可用这些例子直接完成列举，但选择依据和解释机制必须来自观点卡。例子不得承担新的核心原因、事实判断、统计结论或第三张观点卡。任何反方或限制都必须回指已选卡片的现有反向链或限制；没有可回指内容时不得临时编写。最后一句必须简单、直接扣题，且不得加入新理由、例子或转折。
